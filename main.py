...
def process_folder(folder_path):
    ...
    # Рекурсивно викликаємо функцію для всіх вкладених папок
    for item in items:
        if item.is_dir() and item.name.lower() not in ['archives', 'videos', 'audio', 'documents', 'images', 'others']:
            process_folder(item)

        # Видалення порожніх вкладених папок
...
def process_file(file_path, target_path):
    ...
    if file_extension in ['jpeg', 'png', 'jpg', 'svg']:
        ...
        try:
            # Створити папку для розпакованих файлів
            ...
            # Видалити порожню папку розпакованих файлів
    ...
    # Створити папку призначення, якщо вона не існує
...
def main():
    # Отримати аргумент командного рядка для шляху до папки
    if len(sys.argv) != 2:
...
# Запуск основної функції зазначеної папки
main()
